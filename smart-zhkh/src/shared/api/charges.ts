...
export async function deleteCharge(id: number) {
  const res = await fetch(`http://localhost:8080/api/charges/${id}`, {
    method: 'DELETE',
  });
...
export async function markChargeAsPaid(id: number) {
  const res = await fetch(`http://localhost:8080/api/charges/${id}/pay`, {
    method: 'POST',
...
export async function updateCharge(charge: any) {
  const res = await fetch(`http://localhost:8080/api/charges/${charge.id}`, {
    method: 'PUT',
    headers: {
      'Content-Type': 'application/json',
    },
